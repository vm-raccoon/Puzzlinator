...
import solvePuzzle from "./script/solution"

function appendImageStyleForBlock(img) {
    const style = document.createElement("style");
    style.innerHTML = `.block { background-image: url("${img}"); }`;
...
function getRandomTranslate({ container, restricted, size }) {
    ...
    return `translate(${x}px, ${y}px) rotate(${(Math.random() * 360).toFixed(0)}deg)`;
}
    ...
    .then((puzzle) => {
        ...
        $solution.addEventListener("click", async () => {
            ...
            solution.forEach(async (id, idx) => {
                await sleep(30 * idx);
                pieces[id].node.style.transform = `translate(${targets[idx].offsetLeft}px, ${targets[idx].offsetTop}px)`;
            });
        });
    });
